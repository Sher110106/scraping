...
df = pd.read_csv('Software_Product.csv')
...
def fetch_designations(base_url, target_designations):
    ...
    while len(found_designations) < len(target_designations):
        ...
        payload = {
            'api_key': '5faffb20f65bcb9aac361f214d9ca320',
            'url': url,
            'keep_headers': 'true'
        }
...
def main():
    target_designations = ['Product Manager', 'Software Developer', 'Data Analyst',
                           'System Analyst']
    ...
    if os.path.exists('designation_salaries5.csv'):
        results_df = pd.read_csv('designation_salaries5.csv')

    if os.path.exists('unsuccessful_companies5.csv'):
        unsuccessful_companies = pd.read_csv('unsuccessful_companies5.csv')['Company Name'].tolist()
    ...
    for index, row in filtered_df.iterrows():
        ...
        results_df.to_csv('designation_salaries5.csv', index=False)
        pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies5.csv',
                                                                              index=False)

    # Final save of results
    results_df.to_csv('designation_salaries5.csv', index=False)
    pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies5.csv', index=False)
