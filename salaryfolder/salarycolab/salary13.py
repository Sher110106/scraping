...
def fetch_designations(base_url, target_designations):
    ...
    while len(found_designations) < len(target_designations) and page <= max_pages:
        ...
        payload = {
            'api_key': '57a7fad4fcfc42900b996e4c8e76b815',
            'url': url,
            'keep_headers': 'true'
        }
...
def load_cache():
    if os.path.exists('cache9.json'):
        with open('cache9.json', 'r') as f:
            return json.load(f)
    return {}
...
def save_cache(cache):
    with open('cache9.json', 'w') as f:
        json.dump(cache, f)


# Main function to integrate both functionalities
def main():
    target_designations = ['Web Developer', 'SEO Specialist', 'Team Lead',
    ...
    if os.path.exists('designation_salaries14.csv'):
        results_df = pd.read_csv('designation_salaries14.csv')

    if os.path.exists('unsuccessful_companies14.csv'):
        unsuccessful_companies = pd.read_csv('unsuccessful_companies14.csv')['Company Name'].tolist()
    ...
    for index, row in filtered_df.iterrows():
        ...
        results_df.to_csv('designation_salaries14.csv', index=False)
        pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies14.csv', index=False)
    ...
    files.download('designation_salaries14.csv')
    files.download('unsuccessful_companies14.csv')
    files.download('cache9.json')
