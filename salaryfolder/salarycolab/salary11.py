...
def load_cache():
    if os.path.exists('cache7.json'):
        with open('cache7.json', 'r') as f:
            return json.load(f)
    return {}
...
def save_cache(cache):
    with open('cache7.json', 'w') as f:
        json.dump(cache, f)


# Main function to integrate both functionalities
def main():
    target_designations = ['Student Counsellor', 'Professor', 'Teacher',
                           'Accountant']
    ...
    if os.path.exists('designation_salaries12.csv'):
        results_df = pd.read_csv('designation_salaries12.csv')

    if os.path.exists('unsuccessful_companies12.csv'):
        unsuccessful_companies = pd.read_csv('unsuccessful_companies12.csv')['Company Name'].tolist()
    ...
    for index, row in filtered_df.iterrows():
        ...
        results_df.to_csv('designation_salaries12.csv', index=False)
        pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies12.csv', index=False)
    ...
    files.download('designation_salaries12.csv')
    files.download('unsuccessful_companies12.csv')
