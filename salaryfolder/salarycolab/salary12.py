...
def load_cache():
    if os.path.exists('cache8.json'):
        with open('cache8.json', 'r') as f:
            return json.load(f)
    return {}
...
def save_cache(cache):
    with open('cache8.json', 'w') as f:
        json.dump(cache, f)


# Main function to integrate both functionalities
def main():
    target_designations = ['Software Engineer', 'Technical Support Associate', 'Team Lead',
                           'Marketing Executive']
    ...
    if os.path.exists('designation_salaries13.csv'):
        results_df = pd.read_csv('designation_salaries13.csv')

    if os.path.exists('unsuccessful_companies13.csv'):
        unsuccessful_companies = pd.read_csv('unsuccessful_companies13.csv')['Company Name'].tolist()
    ...
    for index, row in filtered_df.iterrows():
        ...
        results_df.to_csv('designation_salaries13.csv', index=False)
        pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies13.csv', index=False)
    ...
    files.download('designation_salaries13.csv')
    files.download('unsuccessful_companies13.csv')
    files.download('cache8.json')
