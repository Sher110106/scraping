...
def fetch_designations(base_url, target_designations):
    ...
    max_pages = 80  # Limit pagination to 80 pages

    while len(found_designations) < len(target_designations) and page <= max_pages:
        url = f"{base_url}?page={page}"
        payload = {
            'api_key': 'e388695450a966b133b56d2d4e5f3399',
            'url': url,
            'keep_headers': 'true'
        }
...
def load_cache():
    if os.path.exists('cache6.json'):
        with open('cache6.json', 'r') as f:
            return json.load(f)
    return {}
...
def save_cache(cache):
    with open('cache6.json', 'w') as f:
        json.dump(cache, f)


# Main function to integrate both functionalities
def main():
    target_designations = ['Staff Nurse', 'Medical Officer', 'Pharmacist',
                           'Doctor']
    ...
    if os.path.exists('designation_salaries11.csv'):
        results_df = pd.read_csv('designation_salaries11.csv')

    if os.path.exists('unsuccessful_companies11.csv'):
        unsuccessful_companies = pd.read_csv('unsuccessful_companies11.csv')['Company Name'].tolist()
    ...
    for index, row in filtered_df.iterrows():
        ...
        results_df.to_csv('designation_salaries11.csv', index=False)
        pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies11.csv', index=False)
    ...
    files.download('designation_salaries11.csv')
    files.download('unsuccessful_companies11.csv')
    files.download('cache6.json')
