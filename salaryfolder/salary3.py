...
df = pd.read_csv('Marketing_and_Advertising.csv')
...
def main():
    target_designations = ['Marketing Manager','Copywriter','Brand Manager','Social Media Manager']  # Replace with your target designations
    ...
    results_df.to_csv('designation_salaries4.csv', index=False)

    # Save the unsuccessful company names to another CSV file
    pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies4.csv', index=False)
