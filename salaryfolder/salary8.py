...
def load_cache():
    if os.path.exists('cache4.json'):
        with open('cache4.json', 'r') as f:
            return json.load(f)
    return {}
...
def save_cache(cache):
    with open('cache4.json', 'w') as f:
        json.dump(cache, f)


# Main function to integrate both functionalities
def main():
    target_designations = ['Store Manager', 'Customer Service Executive', 'Team Lead',
                           'Sales Executive']
    ...
    if os.path.exists('designation_salaries9.csv'):
        results_df = pd.read_csv('designation_salaries9.csv')

    if os.path.exists('unsuccessful_companies9.csv'):
        unsuccessful_companies = pd.read_csv('unsuccessful_companies9.csv')['Company Name'].tolist()
    ...
    for index, row in filtered_df.iterrows():
        ...
        results_df.to_csv('designation_salaries9.csv', index=False)
        pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies9.csv',
                                                                              index=False)

    # Final save of results
    results_df.to_csv('designation_salaries9.csv', index=False)
    pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies9.csv', index=False)
