...
def fetch_designations(base_url, target_designations):
    ...
    while len(found_designations) < len(target_designations):
        ...
        payload = {
            'api_key': '418a18a20bdc91b19b9386ff1ee7e91c',
            'url': url,
            'keep_headers': 'true'
        }
...
def main():
    target_designations = ['Network Engineer', 'RF Engineer', 'Fiber Engineer', 'Telecom Manager']
    ...
    if os.path.exists('designation_salaries6.csv'):
        results_df = pd.read_csv('designation_salaries6.csv')

    if os.path.exists('unsuccessful_companies6.csv'):
        unsuccessful_companies = pd.read_csv('unsuccessful_companies6.csv')['Company Name'].tolist()
    ...
    for index, row in filtered_df.iterrows():
        ...
        results_df.to_csv('designation_salaries6.csv', index=False)
        pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies6.csv',
                                                                              index=False)

    # Final save of results
    results_df.to_csv('designation_salaries6.csv', index=False)
    pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies6.csv', index=False)
