...
def load_cache():
    if os.path.exists('cache3.json'):
        with open('cache3.json', 'r') as f:
            return json.load(f)
    return {}
...
def save_cache(cache):
    with open('cache3.json', 'w') as f:
        json.dump(cache, f)


# Main function to integrate both functionalities
def main():
    target_designations = ['Project Coordinator', 'Business Analyst', 'Program Manager',
                           'HR Officer']
    ...
    if os.path.exists('designation_salaries8.csv'):
        results_df = pd.read_csv('designation_salaries8.csv')

    if os.path.exists('unsuccessful_companies8.csv'):
        unsuccessful_companies = pd.read_csv('unsuccessful_companies8.csv')['Company Name'].tolist()
    ...
    for index, row in filtered_df.iterrows():
        ...
        results_df.to_csv('designation_salaries8.csv', index=False)
        pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies8.csv',
                                                                              index=False)

    # Final save of results
    results_df.to_csv('designation_salaries8.csv', index=False)
    pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies8.csv', index=False)
