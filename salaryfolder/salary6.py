...
def fetch_designations(base_url, target_designations):
    ...
    while len(found_designations) < len(target_designations):
        ...
        payload = {
            'api_key': '5e8e580ea892fb4022727c855b751f65',
            'url': url,
            'keep_headers': 'true'
        }
...
def main():
    target_designations = ['Clinical Pharmacist', 'Medical Representative', 'Regulatory Affairs Manager',
                           'Quality Analyst']
    ...
    if os.path.exists('designation_salaries7.csv'):
        results_df = pd.read_csv('designation_salaries7.csv')

    if os.path.exists('unsuccessful_companies7.csv'):
        unsuccessful_companies = pd.read_csv('unsuccessful_companies7.csv')['Company Name'].tolist()
    ...
    for index, row in filtered_df.iterrows():
        ...
        results_df.to_csv('designation_salaries7.csv', index=False)
        pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies7.csv',
                                                                              index=False)

    # Final save of results
    results_df.to_csv('designation_salaries7.csv', index=False)
    pd.DataFrame(unsuccessful_companies, columns=['Company Name']).to_csv('unsuccessful_companies7.csv', index=False)
